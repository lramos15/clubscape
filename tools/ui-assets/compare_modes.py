...
def main():
    ...
    for kind, source_directory, pattern in [
        ...
        for path in sorted((directory / kind).glob(pattern)):
            ...
            if not source.exists():
                ...
                source = ROOT / "web/ui/evidence/native-modes/original" / source_kind / path.name
                records = json.loads((ROOT / "web/ui/evidence/native-modes/source-inputs.json").read_text())["references"]
    ...
    (directory / "mode-comparison.json").write_text(json.dumps(report, indent=2) + "\n")
