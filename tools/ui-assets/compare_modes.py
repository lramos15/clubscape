...
def main():
    ...
    for kind, source_directory, pattern in lanes:
        ...
        for path in sorted((directory / kind).glob(pattern)):
            ...
            if not source.exists():
                ...
                archive = ROOT / "web/ui/evidence" / ("native-music-controls" if args.music_ui else "native-audio-controls" if args.audio_ui else "native-presentations" if args.presentations else "native-modes")
    ...
    (directory / ("music-ui-comparison.json" if args.music_ui else "audio-ui-comparison.json" if args.audio_ui else "presentation-comparison.json" if args.presentations else "mode-comparison.json")).write_text(json.dumps(report, indent=2) + "\n")
