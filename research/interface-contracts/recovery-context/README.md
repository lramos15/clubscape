...
`counts.entries` counts visible slots. `nativeItemTypes` counts distinct
original item IDs across those slots; null explicitly means source identity is
incomplete. `capacity` and `capacityUnit` come from the canonical death policy:
...
For a selected source item, `selectedTypeCaption` supplies the original
...
TypeScript definitions are in `web/shared/contracts.ts`. The implementation
will publish the public converter
...
typecheck exclusions or casts. Core/protocol/client/WASM completion and
regression evidence follow the early type publication.
...
varp261=12345 and varp263=42. Its native type count16 and caption35/1,470 are
component facts, not a player journey or new source-server capture.
