...
fn load_palette() -> Palette {
    Palette::from_chunks(&std::fs::read(repo_root().join("assets/compiled/render/palette.bin")).unwrap()).unwrap()
}
...
fn read_png_rgb(path: &Path) -> Vec<i32> {
    ...
    buf[..info.buffer_size()].chunks_exact(channels).map(|p| ((p[0] as i32) << 16) | ((p[1] as i32) << 8) | p[2] as i32).collect()
}
...
fn gpu(width: u32, height: u32) -> Option<Gpu> {
    let (adapter, device, queue) = match pollster::block_on(clubscape_renderer::gpu::device::request_native_device()) {
        Ok(v) => v,
        Err(e) => {
    ...
    let raster = GpuRasterizer::new(device, queue, &palette.rgb, &gpu_textures, width, height).unwrap();
    Some(Gpu { raster, palette, textures, adapter_name: adapter.get_info().name })
}

fn cpu_render(state: &RasterState, tris: &[Tri], palette: &Palette, textures: &TextureSet, clear: i32) -> Vec<i32> {
...
fn gpu_render(g: &mut Gpu, state: &RasterState, tris: &[Tri], clear: u32) -> Vec<i32> {
    ...
    assert!(frame.is_complete(), "queue completion callback did not fire before readback");
    pixels
}
...
fn gpu_matches_cpu_on_model_fixtures() {
    ...
    let tree = Model::from_chunks(&std::fs::read(repo_root().join("assets/compiled/render/models/object-1277-model-1570-lit.bin")).unwrap()).unwrap();
    for yaw in [0, 256, 512, 1024] {
        cases.push((format!("tree yaw {yaw}"), tree.clone(), yaw, 250, 750, Some(format!("tree-1277-yaw-{yaw}"))));
    }
    for (npc, seq, frames, y, z) in [(3028, 6181, 16, 240, 650), (3028, 6180, 16, 240, 650), (2063, 5668, 14, 160, 400), (2063, 5666, 8, 160, 400)] {
        for frame in 0..frames {
            let path = repo_root().join(format!("assets/compiled/render/models/baked/npc-{npc}-seq-{seq}-frame-{frame}.bin"));
            if let Ok(bytes) = std::fs::read(&path) {
                cases.push((format!("npc {npc} seq {seq} frame {frame}"), Model::from_chunks(&bytes).unwrap(), 256, y, z, Some(format!("npc-{npc}-sequence-{seq}-frame-{frame}"))));
            }
        }
    }
    ...
    for (name, model, yaw, y, z, capture) in cases {
        ...
        let mut drawer = ModelDrawer { state, palette: &g.palette.rgb, scratch: &mut scratch, alpha_pass: 2 };
        drawer.draw_legacy(&model, 0, yaw, 0, 128, 0, y, z, 0, &mut tris).unwrap();
        ...
        assert_eq!(n, 0, "{name}: GPU differs from CPU in {n} pixels (max channel {max}), first index {first:?}");
        if let Some(capture) = capture {
            let expected = read_png_rgb(&repo_root().join(format!("assets/reference/osrs240/models/{capture}.png")));
            let (n, max, _) = diff_count(&gpu, &expected);
            assert_eq!(n, 0, "{name}: GPU differs from source capture in {n} pixels (max channel {max})");
        }
    }
}
...
const SCENES: &[SceneCase] = &[
    SceneCase { name: "tutorial-starting-house", camera: [5888, -2360, 4992], pitch: 2048, yaw: 0, focal: [3094, 3103], base: [3048, 3056] },
    SceneCase { name: "tutorial-survival-coast", camera: [0, 0, 0], pitch: 2048, yaw: 1024, focal: [3101, 3085], base: [3048, 3032] },
    SceneCase { name: "lumbridge-castle-plaza", camera: [0, 0, 0], pitch: 2048, yaw: 0, focal: [3222, 3218], base: [3168, 3168] },
    SceneCase { name: "lumbridge-river-bridge", camera: [0, 0, 0], pitch: 2048, yaw: 2048, focal: [3223, 3217], base: [3168, 3168] },
    SceneCase { name: "lumbridge-windmill-route", camera: [0, 0, 0], pitch: 2048, yaw: 1536, focal: [3166, 3306], base: [3120, 3240] },
];
...
fn fixture_camera(name: &str) -> [i32; 3] {
    ...
    let record = manifest["original_inputs"].as_array().unwrap().iter().find(|i| i["id"].as_str() == Some(&format!("original.scenes.{name}"))).unwrap();
...
fn gpu_matches_cpu_and_source_on_scene_fixtures() {
    let Some(mut g) = gpu(1920, 1080) else { return };
    eprintln!("adapter: {} timestamps: {}", g.adapter_name, g.raster.timestamps_supported());
    ...
    for case in SCENES {
        ...
        let (n, max, first) = diff_count(&gpu, &cpu);
        let expected = read_png_rgb(&repo_root().join(format!("assets/reference/osrs240/scenes/{}.png", case.name)));
        let (ns, maxs, _) = diff_count(&gpu, &expected);
        eprintln!("{}: {} tris, gpu vs cpu {n} px (max {max}), gpu vs source {ns} px (max {maxs}), {:?} incl. readback, gpu ns {:?}",
            case.name, tris.len(), elapsed, g.raster.last_record.triangles);
        assert_eq!(n, 0, "{}: GPU differs from CPU in {n} pixels (max channel {max}), first {first:?}", case.name);
        assert_eq!(ns, 0, "{}: GPU differs from source capture in {ns} pixels (max channel {maxs})", case.name);
    }
}
