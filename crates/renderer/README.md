...
| `load_floor_defs(bytes)` -> `[underlays, overlays]`, `terrain_rebuilt()` -> JSON/undefined | Loads validated `terrain/floors.bin` before block assembly. Successful block scenes report paint/model counts with no missing definitions; undefined means no block scene is loaded. Missing, malformed or incomplete terrain inputs are errors, not approximate rendering. |
...
| `assemble_scene(baseX, baseY, nowMs)` → missing squares | Requires floor definitions and raw `BTER`/`BSHD` data in every loaded block. Assembles the scene (unexported squares stay empty like unloaded map squares in the original) and restarts the scenery animation clock only on success. With a `WorldView.instanceLayout` only the declared source chunks are copied to their destinations; a source chunk may repeat. Undeclared tiles and corners stay unloaded, and terrain blends only the loaded area, as the original `rl4.fn` template loader does. The scene id becomes `blocks@x,y#<template>`. `quarterTurns != 0` remains rejected: raw terrain is present, but exact turned scenery re-lighting still needs its source inputs. Any failure preserves the previous scene. |
...
* Minimap (`scene::minimap`, `tests/minimap.rs`): all 15 native minimap captures (5 scenes ×
  ...
  **All 15 captures are now exact over the whole 512×512 surface — outer five tiles
  ...
  Live block assembly requires the floor definitions and both raw-terrain chunks; missing
  inputs or referenced floors fail before replacing the current scene, never fall back to
  known-inexact exported lighting. Floor records reject incomplete rows, duplicate IDs and
  invalid source field ranges. Raw terrain checks source short/byte domains, shapes, rotations,
  byte-derived heights and shadow records before indexing. Direct source fixture scenes remain
  supported independently of block assembly.
